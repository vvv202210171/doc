...
| 参数位置 | 参数名 | 类型 | 必填 | 说明 |
|---------|--------|------|------|------|
| Query | id | Long | ✅ | 收款方式 ID |
| Header | token | string | ✅ | 登录令牌 |
...
其中 `your_token` 是通过登录接口 `/member/loginByTelOrEmail` 获取的令牌。
